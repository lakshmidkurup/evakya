...
import re
class acomma3:
    tree1 = ET.parse('D:\\evakya\\xml\\adjcomma.xml') #--The question, Alice answered is difficult and intelligent.
##    tree1 = ET.parse('D:\\evakya\\dataset\\adjcomma3.xml') #--The novel, Mary read is short, interesting and selective.
##    tree2 = ET.parse('D:\\evakya\\dataset\\adjcomma4.xml') #----The poem, Alice wrote is lovely, fascinating, short and precise.
    tree2 = ET.parse('D:\\evakya\\xml\\adjcommap2.xml')#-The job, John got is demanding and ambitious.
##    tree2 = ET.parse('D:\\evakya\\dataset\\adjcommap3.xml')#---The position, Mary achieved is exceptional, high-grade and excellent.
##    tree = ET.parse('D:\\evakya\\dataset\\adjcommap4.xml')#The second-prize, Alice secured is exceptional, high-grade and excellent.
    tree3 = ET.parse('D:\\evakya\\xml\\adjcomman2.xml')#-The leg, Mary hurt is fractured and sprained.
##    tree4 = ET.parse('D:\\evakya\\dataset\\adjcomman3.xml')#The books, Alice lost are precious, expensive and valuable.
##    tree1 = ET.parse('D:\\evakya\\dataset\\adjcomman4.xml')#The books, John lost are precious, expensive, valuable and worthy.
    ...
    def genverb(self):
        for i in acomma3.root:
            if i.tag=="VERB":
                verblst=[]
                acomma3.eclass=i.attrib["ECLASS"]
                ...
                with open("D:\\evakya\\dataset\\verbclass.txt") as nd:
                    for word in nd:
                        dh=word.split(':')
                        ##print(dh)
                        if acomma3.eclass==dh[2]:
                            verblst.append(dh[1])
                acomma3.verb=random.choice(verblst)
                ...
                with open("D:\\evakya\\dataset\\verbclass.txt") as nd:
                    for word in nd:
                        dh=word.split(':')
                        ##print(dh)
                        if acomma3.verb==dh[1]:
                            acomma3.verbclass=dh[0]
                   
                #print(acomma3.verbclass)            
                return(acomma3.verb)
            ...
            if i.tag=="ADJLST":
               ...
               with open("D:\\evakya\\dataset\\adjectives.txt") as nd:
                    for word in nd:
                        dh=word.split(':')
                        ##print(dh)
                        if acomma3.verbclass==dh[0]:

                           acomma3.adlst.append(dh[1])
            adj=[]               
            adj=acomma3.adlst           
            comma=","
            if i.tag=="ADJLST":
...
##            
    def genonoun(self):
        ...
        acomma3.onoun=random.choice(onounlst)
        if acomma3.verbclass=="personified":
            acomma3.onoun="person"
        #print("acomma3.onoun",acomma3.onoun)
        return(acomma3.onoun)
    def genvbz(self):
        for i in acomma3.root:
            if i.tag=="ONOUN":
                #print("onounssss",acomma3.onoun)
              
                if acomma3.onoun.endswith("s"):
                    vbz="are"
                    return(vbz)
                else:
                    vbz="is"
    ...
    def gensent(self):
        ...
        comma=","
        period="."        
        correctans=det+" "+acomma3.onoun+comma+" "+sname+" "+verb+" "+vbz+" "+res+period
        #print("sname",sname)
        ...
        for i, element in enumerate(mylist):
            ...
            if current_element ==verb and next_element==vbz:
                    taglst.append(current_element)
                    taglst.append("<w>")
            if current_element ==vbz and next_element in acomma3.adlst:
                    taglst.append(current_element)
                    taglst.append("<w>")
            if current_element in acomma3.adlst and next_element in acomma3.adlst:
                    taglst.append(current_element)
                    taglst.append("<m>")
            if current_element in acomma3.adlst and next_element ==conj:
                    taglst.append(current_element)
                    taglst.append("<w>")
            if current_element ==conj and next_element in acomma3.adlst:
                    taglst.append(current_element)
                    taglst.append("<w>")
            if current_element in acomma3.adlst and next_element ==None:
                    taglst.append(current_element)
                    taglst.append("<m>")
