...
##            #----------------------------------print no:of attempts--------------------------------------------------------------------------------------------------
            ...
            if no_at==0:
                ...
                hintlst,mandlst,correctlst=fr.fone(b,c,correct,category,level,session['username'],session['id'], eid,submittedtext)
                ...
                print("after nojdjdsjvhsjhfjldhls",no_at) 
                submittedtext = request.form.get("text")
                hintlst=[]
                mandlst=[]
                correctlst=[]
                hintlst,mandlst,correctlst=fr.ftwo(b,c,correct,category,level,session['username'],session['id'], eid,submittedtext)
                ...
                mandlst=[]
                correctlst=[]
                hintlst,mandlst,correctlst=fr.fthree(b,c,correct,category,level,session['username'],session['id'], eid,submittedtext)
                for hint in correctlst:
                    print(hint)
                    flash(hint,"success")
                    score1=0
                ...
                sql_select_query1='select correctans from exercisedetails where rid=%s'
                cursor.execute(sql_select_query1,(maxrid2,))
                ms= cursor.fetchone()
                correctans=ms.get("correctans")
                disp = "Correct Answer is: "+correctans
                flash(disp,"success")
                cursor.execute('insert into exercisedetails(userid,username,exerciseid,correctans,response1,hintmsg,failure,success,score,time1,category,levelp,nattempts) values(%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s)', (session['id'], session['username'], eid,correct,submittedtext,hint,1,0,score1,datetime.datetime.now(),category,level,4))
...
##                mysql.connection.commit()
       ...
       #-------------------------------------------delete failure=0 and success===0-----------------------------------------
        ...
        cursor.execute(sql_select_query1,(eid,))
            #print("UPDATED SUCCESS")
        mysql.connection.commit()
        sql_select_query1='select max(rid) from exercisedetails where exerciseid=%s'
        cursor.execute(sql_select_query1,(eid,))
        mid= cursor.fetchone()
        maxrid2=mid.get("max(rid)")
   
        sql_select_query1='select distinct(nattempts) from exercisedetails where rid=%s'
        cursor.execute(sql_select_query1,(maxrid2,))
        nt= cursor.fetchone()
        print("nt",nt)
        natt=nt.get("nattempts")
        print("natt",natt)
        if natt==1:
             
            
        #-----------------------------------------------max score of previous exercise/attempt--------------------
        
            #sql_select_query1='select max(score) from exercisedetails where nattempts=%s and exerciseid=%s'
        ...
        #-----------------------------------------------get the failure and success per click---------------------
            ...
            cursor.execute(sql_select_query1,(eid,natt,))
            fs= cursor.fetchone()
            ...
            elif (failure==1 or failure>1) and (success>=1):
                score1=maxscore+2
            elif (failure==1 or failure>1) and (success==0):
                score1=maxscore+0    
                
            print("maxscore at attempt 1", failure,success,score1)     
            sql_select_query1='update exercisedetails set score=%s where exerciseid=%s and nattempts=%s'
            cursor.execute(sql_select_query1,(score1,eid,natt,))
            mysql.connection.commit()
         ...
         #----------------------------2nd attempt--------------------------------------------------
            ...
            mid= cursor.fetchone()
            maxrid2=mid.get("max(rid)")
            print("maximum rid at 2",maxrid2)
        #-----------------------------------------------max score of previous click--------------------
            natt2=1
            sql_select_query1='select max(score) from exercisedetails where nattempts=%s'
            cursor.execute(sql_select_query1,(natt2,))
            ms= cursor.fetchone()
            maxscore=ms.get("max(score)")
            if maxscore==None:
                maxscore=0
        ...
        #-----------------------------------------------get the failure and success per click---------------------
            ...
            mysql.connection.commit()
            print("SUCCESS222",failure,success)
        #-----------------------------------------------update the score for second attempt based on failure and success per click---------------------
        ...
        if natt==3:
            ...
            mid= cursor.fetchone()
            maxrid2=mid.get("max(rid)")
            print("maximum rid at 3",maxrid2)
        #-----------------------------------------------max score of prevoius click--------------------
          
            sql_select_query1='select max(score) from exercisedetails where nattempts=%s'
            cursor.execute(sql_select_query1,(natt,))
            ms= cursor.fetchone()
            maxscore=ms.get("max(score)")
            if maxscore==None:
                maxscore=0
                ...
                maxscore=maxscore+0
            print("maxscore for 3",maxscore)
            
 #-----------------------------------------------get the failure and success per click---------------------
        
            sql_select_query1='select sum(failure),sum(success) from exercisedetails where eid=%s and nattempts=%s'
            cursor.execute(sql_select_query1,(eid,natt,))
            fs= cursor.fetchone()
            failure=fs.get("failure")
            success=fs.get("success")
        ...
        #-----------------------------------------------update the score for second attempt based on failure and success per click---------------------
            ...
            elif (failure==1 or failure>1) and (success==0 or success>=1):
                score1=maxscore+0
            #print("score after first attempt",score1)    
            sql_select_query1='update exercisedetails set score=%s where exerciseid=%s and nattempts=%s'
            cursor.execute(sql_select_query1,(score1,eid,natt,))
