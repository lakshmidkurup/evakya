    ...
    if 'loggedin' in session:
        if request.method=="POST":
           ...
           # call(["python", "type16neutral.py"]) #---------------first question on loading the page
            ...
            cursor.execute('insert into exercisedetails(userid,username,exerciseid,correctans,response1,failure,success,score,time1,category,nattempts) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (session['id'], session['username'], eid,correct,inputtext,0,0,0,datetime.datetime.now(),category,0))
...
def submit_textarea():
    ...
    verb=['stand','answered','won','got','secured','achieved','hurt','lost','opened','closed','cooked','met','read','wrote','drew','drank','paid','bought','stayed','worked','joined','taught','married','dated','intracted','consulted','visited','debated','argued','fought','lived','seperated']
    verb1=[]
...
#---------------------------------------------------<m> with fullstop--------------------------------------------------------------------------        
        for i in range(0,len(b)):
            if (b[i]=="<m>" and c[i]=="."):
                ...
                hint="Correct Answer.Fullstop at appropriate place in                                ",submittedtext
                flash(hint,"success")
                correctlst.append(hint)
               
            
                score1=0
                cursor.execute('insert into exercisedetails(userid,username,exerciseid,correctans,response1,failure,success,score,time1,category,nattempts) values(%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (session['id'], session['username'], eid,correct,submittedtext,0,1,score1,datetime.datetime.now(),category,0))
                mysql.connection.commit()
        ...
        for i in range(0,len(b)) :
            ...
            if (b[i]=="<w>" and c[i]==","):
                ...
                if no_at==0:
                    ...
                    for i in hintlst:
                        score1=0    
                        cursor.execute('insert into exercisedetails(userid,username,exerciseid,correctans,response1,failure,success,score,time1,category,nattempts) values(%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (session['id'], session['username'], eid,correct,submittedtext,1,0,score1,datetime.datetime.now(),category,0))
                        mysql.connection.commit()
                ...
                if no_at==1:
                    ...
                    hint="WARNING:CHECK ONCE AGAIN:Is there a need of punctuation[COMMA] in        ",[submittedtext]
                    flash(hint,"warning")
                    hintlst.append(hint)
